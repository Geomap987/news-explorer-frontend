...
function NewsCard({ title, text, date, source, link, image, isOwn, isLiked, onCardLike, onCardDelete, keyWord, theme, isUserLoggedIn }) {
    ...
    return (
        <article className="card-list__card">
            ...
            <button className={isDeleteClicked ? "card-list__delete-window" : "card-list__delete-window_none"}>Убрать из сохраненных</button>
            ...
            {theme === 'searchCards' ? <button onClick={isUserLoggedIn ? handleSaveClick : handleFlagClick} className={isSaved ? "card-list__flag_active" : "card-list__flag"}></button> : <button onClick={handleDeleteClick} className="card-list__delete-icon"></button>}
            <div className="card-list__text">
                <h5 className="card-list__date">{date}</h5>
                <a className="card-list__link" href={link} target="_blank" rel="noreferrer">
